	...
	if activator:Team() == TEAM_HUMAN then
		if self:GetObjectOwner():IsValid() then
			if activator:GetInfo("zs_nousetodeposit") == "0" or activator == self:GetObjectOwner() then
				...
				local togive = math.min(math.min((giveammoamount or 15), activator:GetAmmoCount("pulse")), self:GetMaxAmmo() - curammo)
				if togive > 0 then
...
function ENT:FindZapperTarget(pos, owner)
	...
	for k, ent in pairs(ents.FindInSphere(pos, 135 * (owner.DeployableRangeMul or 1))) do
...
function ENT:Think()
	...
	if curammo >= 2 and owner:IsValid() then
		...
		if target then
			self:SetAmmo(curammo - math.Round(2 * (owner.DeployableOffenseEfficiency or 1), 3))
			if self:GetAmmo() < 1 then
				owner:SendDeployableOutOfAmmoMessage(self)
			end

			self:SetNextZap(CurTime() + 5 * (owner.DeployableDelayMul or 1))

			if owner:IsSkillActive(SKILL_TECH_OFFENSE_RESCAS_ZAPPER) then
			...
			target:TakeSpecialDamage(math.Round(self.Damage * (owner.DeployableDamageMul or 1), 0), DMG_SHOCK, owner, self)
			if self.PointsMultiplier then
