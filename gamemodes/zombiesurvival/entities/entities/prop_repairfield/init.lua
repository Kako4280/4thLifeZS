...
function ENT:Think()
	...
	for _, hitent in pairs(ents.FindInSphere(pos, self.MaxDistance * (owner.FieldRangeMul or 1))) do
		...
		if healed then
			...
			self:SetAmmo(self:GetAmmo() - math.Round(1 * (owner.DeployableRepairEfficiency or 1), 3))
			...
			if count >= 3 or self:GetAmmo() < 1 then break end
		end
	end

	if count > 0 then
		self:SetNextRepairPulse(CurTime() + self.Delay * (self:GetObjectOwner().FieldDelayMul or 1))
	end
