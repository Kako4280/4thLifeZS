...
function EFFECT:Init(data)
	...
	self.Pos = pos
	self.Normal = normal
	self.DieTime = CurTime() + self.LifeTime / 2
	...
	if MySelf:IsValid() then
		...
		if dist < 200 and WorldVisible(eyepos, pos) then
			...
			dir:Normalize()
			power = (power - (1 - math.max(0, EyeVector():Dot(dir)))) / 4
			...
			visionaltermul = visionaltermul / 2
			if power > 0.5 then
				--MySelf:SetDSP(35)
				util.WhiteOut(power * 2 * visionaltermul, 1 * visionaltermul)
			elseif power > 0 then
