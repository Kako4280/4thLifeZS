...
function EFFECT:Render()
	...
	for i = 0, nlen, 32 do
		...
		render.DrawBeam(prevspinpos, spinbeampos, 10, texcoord + i, texcoord + nlen / 128 + i, Color(35, 200, 35, 255 * alpha))

		prevspinpos = spinbeampos
	end

	for i = 0, 4 do
		render.DrawBeam(self.StartPos, self.EndPos, 5, texcoord, texcoord + nlen / 255, Color(0, 175, 0, 255 * alpha))
	end
	render.DrawBeam(self.StartPos, self.EndPos, 10, texcoord, texcoord + nlen / 255, Color(0, 175, 0, 255 * alpha))

	render.SetMaterial(glowmat)
	render.DrawSprite(self.StartPos, 50, 50, Color(20, 100, 20, 148 * alpha))
	render.DrawSprite(self.EndPos, 50, 50, Color(20, 100, 20, 148 * alpha))
end
