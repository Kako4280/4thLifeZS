...
GAMEMODE:AddNewRemantleBranch(SWEP, 1, "'Stormer' Electrical mine layer", "Mines zaps zombies in an arc rather than exploding on trigger", function(wept)
	wept.Primary.Projectile = "projectile_impactmine_electro"
...
function SWEP:CanPrimaryAttack()
	if self.BaseClass.CanPrimaryAttack(self) then
		local c = 0
		for _, ent in pairs(ents.FindByClass(self.Primary.Projectile)) do
...
function SWEP:SecondaryAttack()
	...
	if SERVER then
		for _, ent in pairs(ents.FindInSphere(hitpos, 24)) do
			if ent:GetClass() == self.Primary.Projectile and ent:GetOwner() == owner then
