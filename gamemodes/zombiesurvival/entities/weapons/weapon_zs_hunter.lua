...
SWEP.HunterEffect = true

GAMEMODE:AddNewRemantleBranch(SWEP, 1, "'Scavenger' Birdshot Rifle", "Fires a spread of less accurate shots that deal more total damage", function(wept)
	wept.HunterEffect = false
...
local branch = GAMEMODE:AddNewRemantleBranch(SWEP, 2, "'Paroxysm' Explosive Rifle", "Uses twice as much ammo, reloads slowly, but overkill damage is dealt as an explosion", function(wept)
	wept.HunterEffect = true
...
branch.Colors = {Color(150, 100, 120), Color(185, 55, 105), Color(240, 95, 25)}
branch.NewNames = {"Flared", "Fierce", "Volatile"}
...
function SWEP:SendWeaponAnimation()
	...
	if vm:IsValid() then
		vm:SetPlaybackRate(0.83 * speed)
	end
end

function SWEP.BulletCallback(attacker, tr, dmginfo, inf)
	if inf.HunterEffect == true then
	   local effectdata = EffectData()
