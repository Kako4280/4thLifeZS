...
local branch = GAMEMODE:AddNewRemantleBranch(SWEP, 1, "'Hades' Higgs Gun", "Has a pulse slowing effect but deals less damage", function(wept)
	wept.Primary.Damage = wept.Primary.Damage * 0.70
...
GAMEMODE:AddNewRemantleBranch(SWEP, 2, "'Tartarus' Muon Gun", "Deals more damage with heat, but can overheat and damage you, reduced range", function(wept)
	wept.GluonDamage = function(self)
		return wept.Primary.Damage + (self:GetShortHeat() * 0.50 * wept.Primary.Damage)
	end
	wept.TracerName = "tracer_muon"
	wept.FireSoundPitch = 65
	wept.Overheat = true
	wept.Primary.MaxDistance = 768
...
local branch = GAMEMODE:AddNewRemantleBranch(SWEP, 3, "'Sun' Gluon Gun", "Deals less damage but sets zombies on fire.", function(wept)
	wept.Primary.Damage = wept.Primary.Damage * 0.8
	
	wept.TracerName = "tracer_sunbeam"
	wept.EmitStartFiringSound = function(self)
		self:EmitSound("ambient/machines/teleport1.wav", 75, 210)
		self:EmitSound("npc/combine_gunship/ping_patrol.wav", 75, 120, 1, CHAN_AUTO+20)
	end
	wept.FireSoundPitch = 45
	wept.Primary.MaxDistance = 1028
    wept.Burn = true
end)
...
function SWEP.BulletCallbackEcho(attacker, tr, dmginfo, inf)
	...
	if attacker:GetActiveWeapon().LegDamage then
		if ent:IsValidZombie() then
			ent:AddLegDamageExt(5.5, attacker, attacker:GetActiveWeapon(), SLOWTYPE_PULSE)
		end
	end
