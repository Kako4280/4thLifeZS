...
SWEP.PrintName = "Unstable Ghoul"

SWEP.Base = "weapon_zs_zombie"

SWEP.MeleeDamage = 35
SWEP.MeleeDamageVsProps = 37
...
local function DoFleshThrow(owner, self)
	...
	for k, spr in pairs(PoisonPattern) do
		...
		local ent = ents.Create(k % 3 == 1 and "projectile_ghoulfleshno" or "projectile_poisonflesh_w6ghoul")
		if ent:IsValid() then
			...
			if phys:IsValid() then
				phys:SetVelocityInstantaneous(heading * 700)
			end
		end
	end
