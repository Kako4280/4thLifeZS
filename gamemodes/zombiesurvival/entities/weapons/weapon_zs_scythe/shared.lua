...
GAMEMODE:AddNewRemantleBranch(SWEP, 1, "'Reaper' Great Scythe", "Grants a reaper stack on kill, less base damage", function(wept)
...
wept.OnZombieKilled = function(self, zombie, total, dmginfo)
 ...
 if killer:IsValid() then
		local reaperstatus = killer:GiveStatus("reaper", 8)
...
function SWEP:MeleeSwing()
	...
	for _, trace in ipairs(tr) do
		...
		if ent and ent:IsValid() and i < 3 then
			if SERVER then
