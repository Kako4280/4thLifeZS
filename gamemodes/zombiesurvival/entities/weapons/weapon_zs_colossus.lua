...
GAMEMODE:AddNewRemantleBranch(SWEP, 1, "'Colossus' Explosive Rifle", "Uses twice as much ammo, reloads slowly, but overkill damage is dealt as an explosion", function(wept)
	wept.Primary.ClipSize = 2
	wept.RequiredClip = 2
	wept.ReloadSpeed = 0.83
	wept.Primary.Damage = 160
	...
	wept.OnZombieKilled = function(self, zombie, total, dmginfo)
		...
		if killer:IsValid() and minushp > 10 then
			...
			timer.Simple(0.15, function()
				util.BlastDamagePlayer(killer:GetActiveWeapon(), killer, pos, 72, minushp, DMG_ALWAYSGIB, 0.85)
			end)
