...
function meta:ProcessDamage(dmginfo)
	...
	if P_Team(self) == TEAM_UNDEAD then
		...
		-- if attacker:IsValid() and inflictor:GetClass() == "entityflame" and self:Team() == TEAM_UNDEAD then
			-- local atkr
			...
			-- dmginfo:SetDamage(3 + (atkr.AddedDamageFire or 0))
			-- if IsValid(self:GetZombieClassTable()) then
				-- if IsValid(self:GetZombieClassTable().ResistanceFire) then
					-- dmginfo:SetDamage(math.ceil(dmginfo:GetDamage() * (1 - self:GetZombieClassTable().ResistanceFire)))
				-- end
			-- end
		-- end
		...
		if attacker:IsValidLivingHuman() and inflictor:IsValid() and inflictor == attacker:GetActiveWeapon() then
			...
			local attackermaxhp = math.floor(attacker:GetMaxHealth() * (attacker:IsSkillActive(SKILL_D_FRAIL) and 0.50 or 1))
			...
			if wep.IsMelee then
				...
				--	attacker:SetBloodArmor(math.min(attacker.MaxBloodArmor, attacker:GetBloodArmor() + math.floor(math.min(damage, (self:Health()) * attacker.MeleeDamageToBloodArmorMul * attacker.BloodarmorGainMul * 4)^0.62)))
				--end
				...
				if attacker:HasTrinket("sharpstone") then
					dmginfo:SetDamage(dmginfo:GetDamage() * (1.05))
				end

				if wep.Culinary and attacker:IsSkillActive(SKILL_MASTERCHEF) and math.random(1,6) == 1 then
					self.ChefMarkOwner = attacker
					self.ChefMarkTime = CurTime() + 3
				end
			end
		end
	...
	if self:IsSkillActive(SKILL_BLOODLUST) and attacker:IsValid() and attacker:IsPlayer() and inflictor:IsValid() and attacker:Team() == TEAM_UNDEAD then
		self:SetPhantomHealth(math.min(self:GetPhantomHealth() + dmginfo:GetDamage() / 3.5, self:GetMaxHealth()))
	end
...
function meta:SetPoints(points)
	self:SetDTFloat(1, points)
end

function meta:SetBloodArmor(armor)
	self:SetDTFloat(DT_PLAYER_FLOAT_BLOODARMOR, armor)
end
...
function meta:GiveStatus(sType, fDie)
	...
	if resistable and self:IsSkillActive(SKILL_HAEMOSTASIS) and self:GetBloodArmor() >= 3 then
		self:SetBloodArmor(self:GetBloodArmor() - 3)
		return
	end
...
function meta:Resupply(owner, obj)
	...
	if not ammotype == "scrap" then
		amount = amount * (self.ResupplyAmmoMul or 1)
	end
	...
	for i = 1, 1 do
		...
		self:GiveAmmo(math.floor(amount), ammotype)

		if self:IsSkillActive(SKILL_FORAGER) and math.random(1,1000) <= 150  and #GAMEMODE.Food > 0 then
			self:Give(GAMEMODE.Food[math.random(#GAMEMODE.Food)])
		end
		
		if self:IsSkillActive(SKILL_TECH_RESUPPLY_BONUS_SCRAP) then
			self:GiveAmmo(1, "scrap")
		end
		
		if self:IsSkillActive(SKILL_TECH_RESUPPLY_BONUS_CADE) then
			if math.random(1,3) == 1 then
				self:GiveAmmo(1, "sniperround")
			else
				self:GiveAmmo(1, "gaussenergy")
			end
		end
...
function meta:AddPoints(points, floatingscoreobject, fmtype, nomul)
	...
	if self.PointsVault then
		self.PointsVault = self.PointsVault + math.Round(points, 1) * GAMEMODE.PointSaving
	end

	if floatingscoreobject and points >= 0.1 then
		self:FloatingScore(floatingscoreobject, "floatingscore", points, fmtype or FM_NONE)
	end
	...
	self:SetNWFloat("points", self:GetNWFloat("points") + points)
	...
	if teamid == TEAM_UNDEAD then
		...
		if classtab then
			...
			if classtab.HullDuck then
				if classtab.ModelScale then
					self:SetHullDuck(classtab.HullDuck[1] / classtab.ModelScale, classtab.HullDuck[2] / classtab.ModelScale)
				else
			...
			if classtab.Hull then
				if classtab.ModelScale then
					self:SetHull(classtab.Hull[1] / classtab.ModelScale, classtab.Hull[2] / classtab.ModelScale)
				else
...
function meta:PulseResonance(attacker, inflictor)
	...
	timer.Create("PulseResonance" .. attacker:UniqueID(), 0.06, 1, function()
		...
		if attacker:IsValidLivingHuman() then
			util.BlastDamagePlayer(inflictor, attacker, pos, (inflictor.PulseRange or 100) * (attacker.ResCasIncreasedRadius or 1), (inflictor.PulseDamage or 75) * (attacker.ResCasIncreasedDamage or 1), DMG_ALWAYSGIB, 0.95)
			for _, ent in pairs(util.BlastAlloc(inflictor, attacker, pos, (inflictor.PulseRange or 100) * (attacker.ExpDamageRadiusMul or 1) * (attacker.ResCasIncreasedRadius or 1))) do
