...
WEAPON_MODIFIER_ADDED_BURN_DURATION = 27
...
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_ADDED_BURN_DURATION,		"Burn Duration", 			false, 	{AddedBurnDuration = false})
...
function GM:CreateWeaponOfQuality(i, orig, quality, classname, branch)
	...
	if wept.PrimaryRemantleModifier then
		...
		if wept.Bleed then
			wept.BleedDamage = math.Round(wept.BleedDamage * quality[2], 0)
		end
		if wept.Burn then
			wept.BurnDamage = math.Round(wept.BurnDamage * quality[2], 0)
		end
...
local function BranchCost(wtbl, branch)
	local itier = wtbl.Tier or 1

	local cost = wtbl.ScrapCost or GAMEMODE.ScrapVals[itier] -- Main / regular cost
	local bcost = wtbl.VarScrapCost or GAMEMODE.ScrapVals[itier] -- Branch cost
	local bcost2 = wtbl.VarScrapCost2 or GAMEMODE.ScrapVals[itier] -- 2nd Branch cost ( Branch Cost 2 )
	local branch2 = branch or wtbl.Branch

	if branch2 == 2 then
		return bcost2
	elseif branch2 == 1 then
		return bcost
	else
		return cost
	end
...
function GM:GetDismantleScrap(wtbl, invitem)
	...
	local icost = BranchCost(wtbl, branch)

	local dismantlediv = invitem and 2 or 1
	local baseval = invitem and GAMEMODE.ScrapValsTrinkets[itier or 1] or icost

	local qu = (quatier or 0) + 1
	local basicvalue = ( ( baseval * GAMEMODE.DismantleMultipliers[qu] ) ) - ((quatier or itier) and 0 or 1) -- This makes T1 cost less

	return math.floor((basicvalue * (wtbl.IsMelee and 0.75 or 1)) / (wtbl.DismantleDiv or dismantlediv))
end

function GM:GetRefundScrap(wtbl, invitem)
	local quatier = wtbl.QualityTier or 1
	local itier = wtbl.Tier or 1
															-- You should not refund the base dismantle amount. Take off the base dismantle value for its tier or custom base cost
	local NotCustom = self:GetDismantleScrap(wtbl, invitem) - GAMEMODE.ScrapVals[itier]

	local rfnd = wtbl.RfndScrap and wtbl.RfndScrap[quatier] or NotCustom
															-- 0.8 mutliplier is default
	local mult = wtbl.RfndMult and wtbl.RfndMult[quatier] or 0.8

	return math.floor((rfnd * mult))

end

-- Will finish at later date
--[[function GM:GetDowngradeScrap(wtbl, qualitychoice, branch)
	local itier = wtbl.Tier
	local quatier = wtbl.QualityTier

	local icost = BranchCost(wtbl, branch)

	return math.ceil( icost * qualitychoice * (0.9) ) -- self.ScrapVals[itier or 1] uses old cost by tier system
end]]

function GM:GetUpgradeScrap(wtbl, qualitychoice, branch)
	local itier = wtbl.Tier

	local icost = BranchCost(wtbl, branch)

	return math.ceil( icost * qualitychoice * (wtbl.IsMelee and 0.85 or 1)) -- self.ScrapVals[itier or 1] uses old cost by tier system
end

function GM:PointsToScrap(points)
	return math.Round((points / 2) * 1.15,0)
end
