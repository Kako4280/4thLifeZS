...
function GM:_PostDrawTranslucentRenderables()
	if not self.DrawingInSky then
		...
		self:DrawNailIndicators()
	end
end

function GM:DrawCrateIndicators()
	if P_Team(MySelf) ~= TEAM_HUMAN or not MySelf:IsSkillActive(SKILL_TECH_START) then return end
...
function GM:DrawNailIndicators()

	local eyepos = EyePos()
	local distance, pos, ang, nearest, myteam, pos2
	...
	local nails = ents.FindByClass("prop_nail")

	if not nails then return end
	
	for i, nail in pairs(nails) do
	
		if not nail:IsValid() then continue end
		if not nail:GetParent():IsValid() then continue end
		
		if MySelf:IsValid() then
			myteam = MySelf:Team()
			pos2 = nail:GetParent():GetPos()
			if myteam == TEAM_HUMAN or myteam == TEAM_SPECTATOR then
				drawinfo = (GAMEMODE.AlwaysShowNails or MySelf:KeyDown(IN_SPEED) or GAMEMODE.TraceTargetNoPlayers == nail:GetParent()) and eyepos:DistToSqr(pos2) <= 262144 and WorldVisible(eyepos, pos2)
			elseif myteam == TEAM_UNDEAD then
				drawinfo = GAMEMODE.TraceTargetNoPlayers == nail:GetParent()
			end
		end
		
		local displayowner = nail:GetDTString(0)
		local redname = false
		local expert = false
		local hcolor = COLOR_WHITE
		local deployer = nail:GetOwner()
		...
		if deployer:IsValid() then
			...
			if deployer:Team() == TEAM_HUMAN and deployer:Alive() then
				local olvl = deployer:GetZSLevel()
				local mlvl = MySelf:GetZSLevel()
					
				if olvl >= 4 and (mlvl - 3) < olvl then
					...
					if colLvl > 0 and colLvl <= 25 then
						lvlRed = 250
						lvlGre = 250 - (10 * colLvl)
						lvlBlu = 250 - (10 * colLvl)
					elseif colLvl > 25 and colLvl <= 50 then
						colLvl = colLvl - 25
						lvlRed = 250
						lvlGre = 0 + (10 * colLvl)
						lvlBlu = 0
					elseif colLvl > 50 and colLvl <= 75 then
						colLvl = colLvl - 50
						lvlRed = 250 - (10 * colLvl)
						lvlGre = 250
						lvlBlu = 0
					elseif colLvl > 75 and colLvl <= 100 then
						colLvl = colLvl - 75
						lvlRed = 0
						lvlGre = 250
						lvlBlu = 0 + (10 * colLvl)
					elseif colLvl > 100 and colLvl <= 200 then
						colLvl = colLvl - 100
						lvlRed = 0
		...
		nearest = nail:GetParent():WorldSpaceCenter()
		...
		if vis < 0.01 then continue end
		
		if not drawinfo then continue end
		
		cam_IgnoreZ(true)
		cam_Start3D2D(nearest, ang, 0.1)
			...
			if nail:GetMaxRepairs(MySelf) > 0 or nail:GetMaxNailHealth() > 0 then
				local repairs = nail:GetRepairs()
				local mrps = nail:GetMaxRepairs(MySelf)
				...
				if displayowner then
					...
					draw.SimpleText(math.floor(nhp) .. "/" .. math.floor(nail:GetMaxNailHealth()), "ZS3D2DUnstyleTiny", x + 25, y - 30, col, TEXT_ALIGN_CENTER)
				end
			end
