...
function meta:HealPlayer(pl, amount, pointmul, nobymsg, poisononly)
	...
	local health, maxhealth = pl:Health(), pl:IsSkillActive(SKILL_D_FRAIL) and math.floor(pl:GetMaxHealth() * 0.50) or pl:GetMaxHealth()
...
function meta:DamageNails(attacker, inflictor, damage, dmginfo)
	...
	if self.ReflectEnd and CurTime() < self.ReflectEnd and self.ReflectApplier and self.ReflectApplier:IsValidLivingHuman() then
		local applier = self.ReflectApplier
		
		if attacker:IsValidLivingZombie() and attacker:Team() == TEAM_UNDEAD then
			attacker:TakeSpecialDamage(dmginfo:GetDamage() * 0.7, DMG_DIRECT, applier)
		end	
    end
	...
	if attacker:IsPlayer() then
		...
		local ZScore = math.Round(damage / math.random(16,22), 1)
...
function meta:RemoveNail(nail, dontremoveentity, removedby, forceremoveconstraint)
	...
	self:RecalculateNailBonuses(nail:GetOwner())
...
function meta:RecalculateNailBonuses(pl)
	local max_health = self:GetMaxBarricadeHealth()
	if max_health == 0 then return end

	--Maximum of 1 nail so this is deprecated
