...
function GM:LevelForXP(xp)
	...
	local xp2 = xp
	for i=1,9999 do
		local XPToLevel = self:XPForLevel(i)
		if XPToLevel <= xp2 then
			if i > 1 then
				xp2 = xp2 - XPToLevel
				Level = Level + 1
			end
...
function GM:XPForLevel(level)
	return math.floor(((level) * 10) * (1.04 ^ math.min(level,100)))
end

function GM:TotalXPForLevel(level)
	local TotalXPForLevel = 0
	for i=1,9999 do
		if i <= level then
			if i > 1 then
				TotalXPForLevel = TotalXPForLevel + self:XPForLevel(i)
			end
		else
...
function GM:ProgressForXP(xp)
	local Level = self:LevelForXP(xp)
	if Level >= self.MaxLevel then return 1 end

	local current_level_xp = self:XPForLevel(Level)
	local next_level_xp = self:XPForLevel(Level + 1)
...
function meta:GetZSXP()
	return self:GetDTFloat(DT_PLAYER_FLOAT_XP)
end
...
function meta:GetZSSPTotal()
	local level = math.min(self:GetZSLevel(),100)
	local levelR = level
	local sp = 5 + math.min(level, 20)
	
	sp = sp + math.Clamp(math.floor(math.max((level - 20), 0) / 2), 0, 15)
	sp = sp + math.Clamp(math.floor(math.max((level - 50), 0) / 5), 0, 10)
	
	return math.max(sp, 6)
end
