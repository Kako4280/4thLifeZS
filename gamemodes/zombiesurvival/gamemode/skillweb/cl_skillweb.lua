...
local offsets = {
	[TREE_HEALTHTREE] = {4, 16},
	[TREE_SPEEDTREE] = {4, -12},
	[TREE_GUNTREE] = {17, -7},
	[TREE_MELEETREE] = {17, 8},
	[TREE_TECHNICIAN] = {-16, 8},
	[TREE_SUPPORTTREE] = {-9, -7}
}
...
function PANEL:DoEdgeScroll(deltatime)
	...
	if camera_velocity.y ~= 0 or camera_velocity.z ~= 0 then
		...
		campos.y = math.Clamp(campos.y, -512, 512)
		campos.z = math.Clamp(campos.z, -512, 512)
...
function GM:DrawXPBar(x, y, w, h, xpw, barwm, hm, level)
	...
	local level = MySelf:GetZSLevel()
	local xp = math.floor(MySelf:GetZSXP())
	local txp = xp - self:TotalXPForLevel(level)
	local progress = txp / self:XPForLevel(level + 1)
