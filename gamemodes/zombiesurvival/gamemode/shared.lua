...
GM.SoundDuration = {
	...
	["zombiesurvival/lasthuman5.ogg"] = 300.503,
...
function GM:AddCustomAmmo()
	...
	game.AddAmmoType({name = "repairfield_nanite"})
	game.AddAmmoType({name = "zapper"})
	game.AddAmmoType({name = "zapper_arc"})
	game.AddAmmoType({name = "zapper_kill"})
	game.AddAmmoType({name = "turret_gun"})
...
function GM:PlayerCanBeHealed(pl)
	local maxhp = pl:IsSkillActive(SKILL_D_FRAIL) and math.floor(pl:GetMaxHealth() * 0.50) or pl:GetMaxHealth()
...
function GM:IsSpecialPerson(pl, image)
	...
	if pl:IsUserGroup("moderator") then
		img = "icons/pearlblue.png"
		tooltip = "Moderator"
	elseif pl:IsUserGroup("admin") then
		img = "icons/e.png"
		tooltip = "Admin"
	elseif pl:IsUserGroup("superadmin") then
		img = "icons/overlapblueorange.png"
		tooltip = "Super Admin"
	elseif pl:IsUserGroup("manager") then
		img = "icons/flame.png"
		tooltip = "Manager"
