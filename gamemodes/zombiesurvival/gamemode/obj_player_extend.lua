...
function meta:NearArsenalCrate()
    ...
    for _, ent in pairs(arseents) do
        ...
        if pos:DistToSqr(nearest) <= 10000 and (WorldVisible(pos, nearest) or self:TraceLine(100).Entity == ent) then -- 80^2
            local class = ent:GetClass("prop_obj_sigil")
            if class == "prop_obj_sigil" then
                if not ent:GetDTBool(0) then
                    return true
                else
                    return false
                end
...
function meta:GetBloodArmor()
	return self:GetDTFloat(DT_PLAYER_FLOAT_BLOODARMOR)
end
...
function meta:AddLegDamageExt(damage, attacker, inflictor, type)
	...
	if type == SLOWTYPE_PULSE then
		...
		if SERVER then
			...
			if attacker.AccuPulse > (100 * attacker.ResThresholdMul) then
...
function meta:SetSpeed(speed)
	...
	local runspeed = self:GetBloodArmor() >= 1 and self:IsSkillActive(SKILL_CARDIOTONIC) and speed + 40 or speed
