...
net.Receive("zs_floatscore", function(length)
	...
	local frags = net.ReadFloat(24)
	local flags = net.ReadUInt(8)

	if victim and victim:IsValid() then
		MySelf:FloatingScore(victim, effectname, math.Round(frags, 1), flags)
	end
end)
...
net.Receive("zs_floatscore_vec", function(length)
	...
	MySelf:FloatingScore(pos, effectname, math.Round(frags, 1), flags)
end)
