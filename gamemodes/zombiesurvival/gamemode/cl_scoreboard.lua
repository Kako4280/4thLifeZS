...
function PANEL:Init()
	...
	self.m_RemortCLabel = EasyLabel(self, "Level", "ZSScoreBoardPlayer", COLOR_GRAY)

	self.m_BrainsLabel = EasyLabel(self, "Score", "ZSScoreBoardPlayer", COLOR_GRAY)
	self.m_RemortCZLabel = EasyLabel(self, "Level", "ZSScoreBoardPlayer", COLOR_GRAY)
...
function PANEL:RefreshPlayer()
	...
	local rlvl = pl:GetZSLevel()
	self.m_RemortLabel:SetText(rlvl > 0 and rlvl or "0")
	...
	--	lvlGre = 0
	--	lvlBlu = 250
	--elseif colLvl > 150 and colLvl <= 175 then
	--	colLvl = colLvl - 150
	--	lvlRed = 250 - (10 * colLvl)
	--	lvlGre = 0
	--	lvlBlu = 250 - (10 * colLvl)
	--elseif colLvl > 175 then
	--	colLvl = colLvl - 175
	--	lvlRed = 0
	--	lvlGre = 0
	--	lvlBlu = 0
	...
	local hcolor = Color(lvlRed, lvlGre, lvlBlu, 255)
	
	self.m_PlayerLabel:SetText(name)
	self.m_PlayerLabel:SetColor(hcolor)
	self.m_PlayerLabel:SetAlpha(240)

	local score = 0
	if pl:Team() == TEAM_UNDEAD then
		score = math.Round(pl:GetNWFloat("zscore"),0)
	elseif pl:Team() == TEAM_HUMAN then
		score = math.floor(pl:GetNWFloat("points"))
	end
	
	self.m_ScoreLabel:SetText(score)
	...
	if pl:Team() == TEAM_HUMAN then
		self:SetZPos(-pl:GetNWFloat("points"))
	elseif pl:Team() == TEAM_UNDEAD then
		self:SetZPos(-pl:GetNWFloat("zscore"))	
	end
...
function PANEL:SetPlayer(pl)
	...
	if pl:IsValidPlayer() then
		...
		--self.m_Flash = pl:SteamID() == "STEAM_0:1:17433654"
	else
		self.m_Avatar:SetVisible(false)
		self.m_SpecialImage:SetVisible(false)
	end
