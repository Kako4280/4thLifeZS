...
function GM:AddResources()
	...
	for _, filename in pairs(file.Find("materials/icons/*.png", "GAME")) do
		resource.AddFile("materials/icons/"..filename)
	end
	...
	resource.AddFile("sound/weapons/krissv/fire_1.wav")
	resource.AddFile("sound/weapons/krissv/mp5k_magout.wav")
...
weaponmodelstoweapon["models/food/hotdog.mdl"] = "weapon_zs_f_hotdog"
...
function GM:PlayerShouldTakeNailRemovalPenalty(pl, nail, nailowner, prop)
	...
	if pl:IsUserGroup("moderator") or pl:IsUserGroup("admin") or pl:IsUserGroup("superadmin") or pl:IsUserGroup("owner") or pl:IsUserGroup("legendary") or pl:IsUserGroup("ascended") or pl:IsUserGroup("diamond") or pl:IsUserGroup("ruby") or pl:IsUserGroup("sapphire") or pl:IsUserGroup("emerald") or pl:IsUserGroup("tanzanite") or pl:IsUserGroup("obsidian") or pl:IsUserGroup("topaz") then return false end
	if nailowner.ZSFriends[pl] then return false end

	if not gamemode.Call("CanPlaceNail", nailowner) then return false end

	
	local playLevel = pl:GetZSLevel()
	local nailLevel = nailowner:GetZSLevel()
	if playLevel > 4 and (nailLevel - 3) < playLevel then -- If I'm better than they are.
		return false
	end
...
local function BossZombieSort(za, zb)
	local ascore = za:GetNWFloat("zscore")
	local bscore = zb:GetNWFloat("zscore")
...
-- function GM:SpawnMiniBossZombie(bossplayer, silent, bossindex, triggerboss)
	...
	-- bossplayer:UnSpectateAndSpawn()
	-- bossplayer.DeathClass = curclass
	-- bossplayer.BossHealRemaining = 0
-- end
...
function GM:ShouldRestartRound()
	...
	sql.Query("UPDATE MapTrack SET MapPlays=MapPlays+1 WHERE MapName='"..game.GetMap().."';")
...
function GM:Think()
	...
	if not self.RoundEnded then
		...
		elseif self:GetWaveStart() ~= -1 then
			...
			and (self.BossZombiePlayersRequired <= 0 or #player.GetAll() >= 3) then
		...
		for _, pl in pairs(allplayers) do
			if P_Team(pl) == TEAM_HUMAN and P_Alive(pl) then
				...
				if pl:IsSkillActive(SKILL_BLOODARMOR) and pl.MaxBloodArmor > 0 and time >= pl.NextBloodArmorRegen and pl:GetBloodArmor() < pl.MaxBloodArmor then
					...
					pl:SetBloodArmor(math.min(pl.MaxBloodArmor, pl:GetBloodArmor() + math.floor(pl.MaxBloodArmor / 5 * pl.BloodArmorRegeneration)))
				end
...
function GM:PlayerSwitchWeapon(pl, old, new)
	...
	local weapon = weapons.Get(new:GetClass())

	if weapon.Material then
		PrintMessage( HUD_PRINTTALK, tostring(weapon.Material))
		new:SetSubMaterial(1, weapon.Material)
	end
end
...
function GM:SBScoreCheck(pl)
	if pl:IsValid() then
		return pl:GetNWInt("points")
	end
end
...
function GM:PlayerRepairedObject(pl, other, health, wep)
	...
	local nail = other:GetNails()
	if nail[1]:GetOwner():IsPlayer() then
		if pl == nail[1]:GetOwner() then
			nail[1]:GetOwner():AddPoints(health / 100)
		else
			nail[1]:GetOwner():AddPoints(health / 40)
		end
	else
		nail[1]:SetOwner(pl)
	end
...
function GM:EndRound(winner)
	...
	for k, v in pairs(player.GetAll()) do
		if not v:IsBot() then
			local zscore = v:GetNWFloat("zscore")
			if zscore > 0 then
				v:AddZSXP(math.Round(zscore * 0.3, 0))
			end
		end
	end
	...
	local MapData = sql.QueryRow("SELECT * FROM MapTrack WHERE MapName='"..game.GetMap().."';")
		
	if winner == TEAM_HUMAN then
		MapData.MapWins = MapData.MapWins + 1
	...
	elseif winner == TEAM_UNDEAD then
		MapData.MapLosses = MapData.MapLosses + 1
		if wave == 1 then
			MapData.MapLossesW1 = MapData.MapLossesW1 + 1
			sql.Query("UPDATE MapTrack SET MapLossesW1="..MapData.MapLossesW1.." WHERE MapName='"..game.GetMap().."';")
		elseif wave == 2 then
			MapData.MapLossesW2 = MapData.MapLossesW2 + 1
			sql.Query("UPDATE MapTrack SET MapLossesW2="..MapData.MapLossesW2.." WHERE MapName='"..game.GetMap().."';")
		elseif wave == 3 then
			MapData.MapLossesW3 = MapData.MapLossesW3 + 1
			sql.Query("UPDATE MapTrack SET MapLossesW3="..MapData.MapLossesW3.." WHERE MapName='"..game.GetMap().."';")
		elseif wave == 4 then
			MapData.MapLossesW4 = MapData.MapLossesW4 + 1
			sql.Query("UPDATE MapTrack SET MapLossesW4="..MapData.MapLossesW4.." WHERE MapName='"..game.GetMap().."';")
		elseif wave == 5 then
			MapData.MapLossesW5 = MapData.MapLossesW5 + 1
			sql.Query("UPDATE MapTrack SET MapLossesW5="..MapData.MapLossesW5.." WHERE MapName='"..game.GetMap().."';")
		elseif wave == 6 then
			MapData.MapLossesW6 = MapData.MapLossesW6 + 1
			sql.Query("UPDATE MapTrack SET MapLossesW6="..MapData.MapLossesW6.." WHERE MapName='"..game.GetMap().."';")
		end
	...
	local MapLossesWeighted = (MapData.MapLossesW1 * 2)+(MapData.MapLossesW2 * 1.5)+(MapData.MapLossesW3 * 1.2)+(MapData.MapLossesW4 * 1)+(MapData.MapLossesW5 * 0.8)+(MapData.MapLossesW6 * 0.7)
	local DifCalc = math.Round(100-(100*MapData.MapWins/(MapData.MapWins+MapLossesWeighted)))
	
	if MapData.MapWins == 0 then
		DifCalc = 100
	end
	
	sql.Query("UPDATE MapTrack SET MapWins="..MapData.MapWins..", MapLosses="..MapData.MapLosses..", MapDifficulty="..DifCalc.." WHERE MapName='"..game.GetMap().."';")
...
function GM:PlayerInitialSpawnRound(pl)
	...
	if self.PreviouslyDied[uniqueid] or spawnAsTeam == TEAM_UNDEAD then
...
function GM:PlayerDisconnected(pl)
	...
	if not pl:IsBot() then
		local zscore = pl:GetNWFloat("zscore")
		if zscore > 0 then
			pl:AddZSXP(math.Round(zscore * 0.3, 0))
			pl:SetNWFloat("zscore", 0)
		end
	end
...
function GM:CanRemoveOthersNail(pl, nailowner, ent)
	...
	if pl == nailowner then
		return true
	end

	--Set up variables and list of staff ranks, order has to be lowest to highest unless you edit this.
	local staffTable = {"cmoderator", "moderator", "admin", "superadmin", "manager", "owner"}
	local plRank = 0
	local noRank = 0
	
	--Fetch ranks of player and nailowner.
	for k, v in pairs(staffTable) do
		if pl:IsUserGroup(v) then
			plRank = k
		end
		if nailowner:IsUserGroup(v) then
			noRank = k
		end
		if plRank > 0 and noRank > 0 then
			break
		end
	end
	
	--If players rank is higher than nailowners, they can unnail, if not, normal rules apply.
	if plRank > 0 or noRank > 0 then
		if plRank > noRank then 
			return true 
		end
	end
	
	--Check if user gave other player access to unnailing props.
	if nailowner.ZSFriends[pl] then 
		return true 
	end
	
	if not nailowner:IsValid() then
		return true
	end
	
	--Set up variables for user levels.
	local ownerLevel = nailowner:GetZSLevel()
	local selfLevel = pl:GetZSLevel()

	--Checks if the player is a higher level than the nailowner.
	if selfLevel > ownerLevel then
		return true
	else
	...
	--If nothing else, we just return false.
	return true
end
...
function GM:GiveDefaultOrRandomEquipment(pl)
	...
	if pl:IsSkillActive(SKILL_TECH_START) then
		DoDropTechnician(pl)
	end
end
					...
					if damage > 0 then
						...
						if myteam == TEAM_UNDEAD then
							if otherteam == TEAM_HUMAN then
								...
								local ZScore = math.Round(damage / 2.5, 1)
						...
						elseif myteam == TEAM_HUMAN and otherteam == TEAM_UNDEAD then
							...
							if time >= ent.m_LastWaveStartSpawn + 3 and time >= ent.m_LastGasHeal + 2 then
								...
								if not ent:IsBot() and ent:IsPlayer() then
									local wave = self:GetWave()
									if wave == 1 then
										points = points * 2.00
									elseif wave == 2 then
								...
								local MapData = sql.QueryRow("SELECT * FROM MapTrack WHERE MapName='"..game.GetMap().."';")
								local difficulty = MapData.MapDifficulty
								if tonumber(difficulty) > 80 then
									points = points * tonumber(difficulty / 80)
								end
...
function GM:PlayerUse(pl, ent)
	...
	elseif entclass == "item_healthcharger" then
		if pl:Team() == TEAM_UNDEAD then
			return false
		elseif pl:IsSkillActive(SKILL_D_FRAIL) and pl:Health() >= math.floor(pl:GetMaxHealth() * 0.50) then
			return false
		end
...
function GM:HumanKilledZombie(pl, attacker, inflictor, dmginfo, headshot, suicide)
	...
	if inflictor:IsValid() and inflictor == attacker:GetActiveWeapon() then
		...
		local BloodArmorMax = attacker.MaxBloodArmor
		
		if attacker.BloodArmorMeleeKill and wep.IsMelee and BloodArmorMax > 0 then
			local BloodArmorCur = attacker:GetBloodArmor()
			local BloodArmorGain = (attacker.BloodArmorMeleeKill or 0) * (attacker.BloodArmorMeleeKillMul or 1)
			
			if BloodArmorCur <= BloodArmorMax then
				attacker:SetBloodArmor(BloodArmorCur + math.ceil(BloodArmorGain))
			else
				attacker:SetBloodArmor(BloodArmorCur + math.ceil(BloodArmorGain * BloodArmorMax / BloodArmorCur))
			end
		end
...
function GM:ZombieKilledHuman(pl, attacker, inflictor, dmginfo, headshot, suicide)
	...
	attacker:SetNWFloat("zscore",attacker:GetNWFloat("zscore") + 25)
...
function GM:DoPlayerDeath(pl, attacker, dmginfo)
	...
	if plteam == TEAM_UNDEAD then
		...
		if not suicide then
			pl:SetNWFloat("zscore", pl:GetNWFloat("zscore") + 4)
		end
		...
		if not classtable.Boss and pl:GetZombieClass() ~= "Crow" and not suicide then
			DoDrop(pl, atkr, normalDrops)
		elseif classtable.Boss and not suicide then
			local numDrops = 3
			local numBossDrops = 1
			local humanCount = 0
			...
			if humanCount > 0 and atkr:Team() == TEAM_HUMAN then
			numDrops = math.min(3 + math.floor((humanCount - 1) / 3), 8)
			numBossDrops = math.min(1 + math.floor((humanCount) / 8), 3)
			
			numDrops = numDrops + math.random(math.floor(numDrops * (lootMultiplier - 1)), math.ceil(numDrops * (lootMultiplier - 1)))
			numBossDrops = numBossDrops + math.random(math.floor(numBossDrops * (lootMultiplier - 1)), math.ceil(numBossDrops * (lootMultiplier - 1)))
		
				for i = 1, (numDrops + numBossDrops + 1) do
					timer.Simple((i - 1)/10, function()
						if i <= numBossDrops then
							DoDrop(pl, atkr, bossDrops)
						else
							DoDrop(pl, atkr, normalDrops)
						end
					end)
...
function GM:PlayerSpawn(pl)
	...
	if pl:Team() == TEAM_UNDEAD then
		...
		elseif classtab.UsePlayerModel then
			local mdl = string.lower(player_manager.TranslatePlayerModel(pl:GetInfo("cl_playermodel")))
			pl:SetModel(mdl)
		elseif classtab.UsePreviousModel then
			local curmodel = string.lower(pl:GetModel())
			pl:SetModel(curmodel)
		...
		if classtab.Boss then
			...
			local WaveMult = self:GetWave()
			local BaseHealth = classtab.Health
			local BaseHealthAdjusted = math.floor((BaseHealth * 0.5) + ((BaseHealth * 1.04 ^ LivingPlayers) - BaseHealth) + (LivingPlayers * 100) + 200)
			local FinalHealth = BaseHealthAdjusted * ((WaveMult * 0.35) + 0.1)
			
			pl:SetHealth(FinalHealth)
			pl:SetMaxHealth(FinalHealth)
		else
	...
	elseif pl:Team() == TEAM_HUMAN then
		...
		local desiredname = pl:GetInfo("zs_playermodel")
		local desiredskin = pl:GetInfo("zs_playerskin")
		...
		if table.HasValue(self.RestrictedModels, lowermodelname) then
			modelname = "models/player/kliener.mdl"
...
function MapTracker()
	if !sql.TableExists("MapTrack") then
		sql.Query("CREATE TABLE MapTrack(MapName TEXT, MapPlays INTEGER, MapWins INTEGER, MapLosses INTEGER, MapLossesW1 INTEGER, MapLossesW2 INTEGER, MapLossesW3 INTEGER, MapLossesW4 INTEGER, MapLossesW5 INTEGER, MapLossesW6 INTEGER, MapDifficulty INTEGER);")
	end
	
	local CurrentMap = sql.QueryRow("SELECT MapName FROM MapTrack WHERE MapName='"..game.GetMap().."';")
	if !CurrentMap then
		sql.Query("INSERT INTO MapTrack(MapName, MapPlays, MapWins, MapLosses, MapLossesW1, MapLossesW2, MapLossesW3, MapLossesW4, MapLossesW5, MapLossesW6, MapDifficulty) VALUES('"..game.GetMap().."', 1, 0, 0, 0, 0, 0, 0, 0, 0, 100);")
	else
		sql.Query("UPDATE MapTrack SET MapPlays=MapPlays+1 WHERE MapName='"..game.GetMap().."';")
	end	
end
