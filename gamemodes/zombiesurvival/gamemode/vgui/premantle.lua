...
local function RefundClick()
	Derma_Query("Refund upgrades? 80% of invested scrap is returned to you", "Confirm refunding upgrades",
	"Refund", function()
		RunConsoleCommand("zs_refund", SelectedInv() )
	...
	if oldquality ~= hovquality or oldbranch ~= hovbranch then
		...
		if hovquality and hovbranch then
			...
			if quals then
				...
				scost = GAMEMODE:GetUpgradeScrap(self.GunTab, hovquality, hovbranch)
			end
...
net.Receive("zs_remantleconf", function()
	...
	local scost = GAMEMODE:GetUpgradeScrap(gtbl, desiredqua, hovbranch)
...
function PANEL:OnMousePressed(mc)
	if mc == MOUSE_LEFT and hovquality and hovbranch and hovquality ~= 0 then
		...
		if cqua and hovquality > cqua and prev and prev.Unlocked and not current.Locked then
			...
			local scost = GAMEMODE:GetUpgradeScrap(self.GunTab, hovquality, hovbranch)
			if MySelf:GetAmmoCount("scrap") >= scost then
				RunConsoleCommand("zs_upgrade", hovbranch ~= 0 and hovbranch, hovbranch)
				GAMEMODE.RemantlerInterface:Close()
				timer.Simple( (1/33), function() GAMEMODE:OpenRemantlerMenu( MySelf:NearestRemantler() ) end )
				return
			else
...
function GM:OpenRemantlerMenu(remantler)
	...
	for frameindex = 0, 1 do
		...
		if frameindex == 0 then
			...
			for j, tab in ipairs(GAMEMODE.Items) do
				if tab.PointShop and tab.Category == ITEMCAT_TRINKETS and !(tab.BannedFromRemantler) then
					self:AddShopItem(tabpane.Grids[tab.SubCategory], j, tab, false, true)
				end
			end
		else
			local itemframe = vgui.Create("DScrollPanel", ammoframe)
			...
			list:SetPos(8, 16)
			list:SetWide(curframe:GetWide() - 16)
			list:SetTall(curframe:GetTall() - 32)
	...
	compdisl:CenterHorizontal(0.325)
	...
	local canrfnd = true
	local qual = gtbl and gtbl.QualityTier or 0

	if qual > 0 then
		canrfnd = false
	end

	local rfndbtn = EasyButton(remantleframe, "Refund Upgrades", 8, 4)
	rfndbtn:SetFont("ZSHUDFont")
	rfndbtn:SizeToContents()
	rfndbtn:MoveBelow(upgpathf, 32 * screenscale)
	rfndbtn:CenterHorizontal(0.650)
	rfndbtn.DoClick = RefundClick
	if not gtbl then
		rfndbtn:SetDisabled(true)
	elseif canrfnd then
		rfndbtn:SetDisabled(true)
	else
		rfndbtn:SetDisabled(gtbl.NoDismantle)
	end
	rfndbtn:SetTextColor(gtbl and gtbl.NoDismantle and COLOR_DARKGRAY or canrfnd and COLOR_DARKGRAY or gtbl and COLOR_WHITE or COLOR_DARKGRAY)
	frame.m_DisaButton = rfndbtn

	local rfndtxt = ""
	if gtbl then
		local retscrap = self:GetRefundScrap(gtbl, SelectedInv())
		rfndtxt = gtbl.NoDismantle and "Cannot Refund" or canrfnd and "Cannot Refund" or "Refund for " .. retscrap .. " Scrap"
	end

	local rfndscrap = EasyLabel(remantleframe, rfndtxt, "ZSHUDFontSmaller", canrfnd and COLOR_DARKGRAY or COLOR_WHITE)
	rfndscrap:MoveBelow(rfndbtn, 4 * screenscale)
	rfndscrap:CenterHorizontal(0.650)
	frame.m_Refund = rfnddisscrap

	local breakdowns = self.Breakdowns[frame.m_WepClass]
	local compdistxt = breakdowns and self.ZSInventoryItemData[breakdowns.Result].PrintName or ""

	local comprfndl = EasyLabel(remantleframe, compdistxt, "ZSHUDFontSmaller", canrfnd and COLOR_DARKGRAY or COLOR_WHITE)
	comprfndl:MoveBelow(rfndscrap, 4 * screenscale)
	comprfndl:CenterHorizontal(0.650)
	frame.m_ComponentRfnd = comprfndl
